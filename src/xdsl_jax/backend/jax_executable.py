...
def array(object: Any, dtype: DTypeLike | None = None, copy: bool = True) -> Array:
    ...
    JAX type annotations are currently broken, as they don't provide a generic parameter
    to `np.dtype`.
...
class JaxExecutable:
    ...
    def compile(module: ModuleOp) -> "JaxExecutable":
        ...
        loaded = client.compile_and_load(
            bytecode,
        )
    ...
    def __call__(self, stub: Callable[P, R]) -> Callable[P, R]:
        ...
        if len(sig.parameters) != len(operand_types):
            raise ValueError(
                f"Number of parameters ({len(sig.parameters)}) does not match the "
                f"number of operand types ({len(operand_types)})"
            )
        ...
        if sig_return_origin is not tuple:
            if sig.return_annotation is not jnp.ndarray:
                raise NotImplementedError(
                    f"Return annotation is must be `jnp.ndarray` or a tuple of "
                    f"`jnp.ndarray`, got {sig_return}."
                )
            if len(result_types) != 1:
                raise ValueError(
                    f"Number of return values ({len(result_types)}) does not match the "
                    "stub's return annotation"
                )
