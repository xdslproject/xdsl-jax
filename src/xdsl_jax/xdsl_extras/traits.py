...
class SameOperandsAndResultElementType(OpTrait):
    """Constrain the operation to have the same element type for all operands and
    results.
    """

    def verify(self, op: Operation) -> None:
        """Verify that the operation has the same element type for all operands and
        results.
        """
...
class Elementwise(OpTrait):
    ...
    def verify(self, op: Operation) -> None:
        ...
        if len(result_mappable_types) != len(op.results):
            raise VerifyException(
                f"'{op.name}': if an operand is non-scalar, then all results must be "
                "non-scalar"
            )
        ...
        # Check that all types have the same base type
        # (e.g., all TensorType or all VectorType)
        first_type = all_types[0]
        if not all(isinstance(t, type(first_type)) for t in all_types[1:]):
            raise VerifyException(
        ...
        # Check that all types have compatible shapes
        for type_to_check in all_types[1:]:
            if not have_compatible_shape(first_type, type_to_check):
                raise VerifyException(
                    f"'{op.name}': all non-scalar operands/results must have "
                    "compatible shapes"
                )
    ...
    def is_mappable_type(attr_type: Attribute) -> bool:
        ...
        There is a TODO in MLIR to generalize this trait to avoid hardcoding
        vector/tensor. We should update this when the TODO is resolved.
        """
    ...
    def verify(self, op: Operation) -> None:
        ...
        try:
            ...
            for name in self.attr_names:
                ...
                if value is None:
                    return
                results.append(self.operator(value))
        except (TypeError, ValueError, AttributeError) as e:
            raise VerifyException(
                f"cannot compute {self.summary} for {{{names_str}}}: {e}"
            ) from e
