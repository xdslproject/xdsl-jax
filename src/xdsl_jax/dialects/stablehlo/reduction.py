...
# pyright: reportUnknownArgumentType=false, reportUnknownVariableType=false, reportUnknownMemberType=false, reportAttributeAccessIssue=false, reportUnnecessaryComparison=false, reportOptionalMemberAccess=false
...
from xdsl_jax.dialects import _stablehlo_upstream as xstablehlo
    ...
    def verify_(self):
        ...
        for it, iv in zip(input_types, init_types):
            ...
            if it_elem != iv_elem:
                raise VerifyException(
                    "input and init_value must have the same element type"
                )
        ...
        for i, it in enumerate(input_types):
            ...
            if rty.get_element_type() != it_elem:
                raise VerifyException(
                    "reducer return element types must match input element type"
                )
