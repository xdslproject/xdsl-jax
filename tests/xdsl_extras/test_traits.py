...
import pytest
from xdsl.dialects.builtin import TensorType, f32, i32, i64
...
def test_same_operands_element_type_trait():
    ...
    op = ShapeTestOp.create(
        operands=[operand1, operand2], result_types=[TensorType(i32, [2, 2])]
    )
    op.verify()

    operand3 = create_ssa_value(TensorType(f32, [2, 3]))
    op = ShapeTestOp.create(
        operands=[operand1, operand3], result_types=[TensorType(f32, [2, 3])]
    )

    with pytest.raises(
        VerifyException, match="requires the same element type for all operands"
    ):
        op.verify()
...
def test_elementwise_trait():
    ...
    op = ElementwiseTestOp.create(
        operands=[operand], result_types=[TensorType(i32, [2, 3])]
    )
    op.verify()
    ...
    op = ElementwiseTestOp.create(
        operands=[scalar_operand, tensor_operand],
        result_types=[TensorType(i32, [2, 3])],
    )
    op.verify()
...
def test_elementwise_trait_failure_shape_mismatch():
    ...
    with pytest.raises(
        VerifyException,
        match="all non-scalar operands/results must have compatible shapes",
    ):
        op.verify()


def test_elementwise_trait_failure_base_type_mismatch():
    """Test that Elementwise trait fails when mixing TensorType and VectorType."""
    from xdsl.dialects.builtin import VectorType

    operand = create_ssa_value(TensorType(i32, [2, 3]))
    op = ElementwiseTestOp.create(
        operands=[operand], result_types=[VectorType(i32, [2, 3])]
    )

    with pytest.raises(
        VerifyException,
        match="all non-scalar operands/results must have the same base type",
    ):
        op.verify()
...
def test_all_shapes_match():
    ...
    op = ShapeMockOp.create(
        attributes={"a": TensorType(i64, [2, 3]), "b": TensorType(i64, [2, 3])}
    )
    ...
    with pytest.raises(
        VerifyException, match=r"all of \{a, b\} must have the same shape"
    ):
        trait.verify(op)
...
def test_all_ranks_match():
    ...
    op = RankMockOp.create(
        attributes={"a": TensorType(i64, [2, 3]), "b": TensorType(i64, [4, 5])}
    )
    ...
    with pytest.raises(
        VerifyException, match=r"all of \{a, b\} must have the same rank"
    ):
        trait.verify(op)
...
def test_all_element_types_match():
    ...
    trait = AllMatchSameOperatorTrait(
        ("a", "b"), lambda a: a.get_element_type(), "element type"
    )
    trait.verify(op)
    ...
    with pytest.raises(
        VerifyException, match=r"all of \{a, b\} must have the same element type"
    ):
        trait.verify(op)
...
def test_all_element_counts_match():
    ...
    trait = AllMatchSameOperatorTrait(
        ("a", "b"), lambda a: a.element_count(), "element count"
    )
    trait.verify(op)
    ...
    with pytest.raises(
        VerifyException, match=r"all of \{a, b\} must have the same element count"
    ):
        trait.verify(op)
