...
// CHECK-GENERIC: %[[CUSTOM_CALL_LAYOUTS:.*]] = "stablehlo.custom_call"(%[[ARG]]) <{api_version = 4 : i32, backend_config = {bar = 42 : i32}, call_target_name = "bar", has_side_effect = false, operand_layouts = [dense<[1, 0]> : tensor<2xindex>], output_operand_aliases = [], result_layouts = [dense<[1, 0]> : tensor<2xindex>]}> : (tensor<2x2xf32>) -> tensor<2x2xf32>
...
// CHECK-GENERIC: %[[CUSTOM_CALL_TOKEN:.*]] = "stablehlo.custom_call"(%[[TOKEN_INPUT]]) <{api_version = 1 : i32, backend_config = "opaque-config", call_target_name = "token_passthrough", has_side_effect = false, operand_layouts = [dense<> : tensor<0xindex>], output_operand_aliases = [], result_layouts = [dense<> : tensor<0xindex>]}> : (!stablehlo.token) -> !stablehlo.token
...
// CHECK-GENERIC-SAME:     output_tuple_indices = [0],
// CHECK-GENERIC-SAME:     operand_index = 0,
// CHECK-GENERIC-SAME:     operand_tuple_indices = [1]
// CHECK-GENERIC-SAME:   >], has_side_effect = false, api_version = 1 : i32}> : (tuple<tensor<1x1xf32>, tensor<2x3xf32>>, tensor<5x5xf32>) -> (tensor<2x3xf32>, tensor<5x5xf32>)
